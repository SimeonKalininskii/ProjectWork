...
def VRP():
    ...
    df_vrp_filtered = df_vrp_filtered.drop(df_vrp_filtered.columns[1], axis=1)
...
def wage():
    ...
    df_wage = df_wage.drop(df_wage.columns[1], axis=1)
...
def unemployment():
    ...
    df_unemployment = df_unemployment.drop(df_unemployment.columns[1], axis=1)
...
def capture_of_air_pollutants():
    ...
    df_capture_of_air_pollutants = df_capture_of_air_pollutants.drop(df_capture_of_air_pollutants.columns[1],axis=1) #Удалание данных за 2000
...
def emissions_of_pollutants():
    ...
    df_emission_of_pollutants = df_emission_of_pollutants.drop(df_emission_of_pollutants.columns[1], axis=1)
